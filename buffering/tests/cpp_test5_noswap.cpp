              ...
              uint32_t nprod)
{
    ...
    while (i < nrbufs) {
        ...
        cls_put_vector_noswap_all(bufservice, handle, offsetv, countv, VEC_SIZE, buf, nprod);
              ...
              uint32_t ncons)
{
    ...
    while (i < nrbufs) {
        ...
        char *results[VEC_SIZE];
        auto start_time = steady_clock::now();
        cls_get_vector_noswap_all(bufservice, handle, offsetv, countv, VEC_SIZE, results, ncons);
        auto end_time = steady_clock::now();
        ...
        for (uint32_t j = 0; j < VEC_SIZE; ++j) {
            ...
            write(fd, results[j], countv[j]);
        }

        cls_release_buf(bufservice, handle, ncons);
